...
console.log('Running jBart server on port ' + port);
...
extend(op_post_handlers, {   
    'SaveFile': function(req, res,body,path,user_machine) {
        ...
        var normalized_path = path.replace(/\/cygdrive\/c\//g,'c:\\'); // nodejs can not use cygwin path (same on unix)
...
function serveFile(req,res,path) {
  ...
  if (path.match(/^jbart5\//))
    full_path = path.replace(/^jbart5\//,'c:\\jbart5\\');
  if (path.match(/^root/))
    full_path = path.replace(/^root\//,'..\\..\\');
  if (path.match(/^ajaxart/))
    full_path = path.replace(/^ajaxart\//,'..\\');
  if (path.match(/^jbart/))
    full_path = path.replace(/^jbart\//,'..\\');
  if (path.match(/^widgets/))
...
if (client) {
  if (client == 'crossix')
    var js_file = 's:/GUI/nodejs/crossix.js';
  else
