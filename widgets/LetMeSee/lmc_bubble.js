...
function aa_lmc_initMobileBookmarkBubble(identifier) 
{
  // first check localStorage exists
  try {
    localStorage['delme'] = '123';
    localStorage.removeItem('delme');
  } catch(e) {
    // localStorage is not active - maybe iphone5
    return;
  }
...
google.bookmarkbubble.Bubble.prototype.LOCAL_STORAGE_PREFIX = 'LMC_BOOKMARK_';
...
google.bookmarkbubble.Bubble.prototype.MOBILE_SAFARI_USERAGENT_REGEX_ = /iPhone|iPod|iPad|Android|BlackBerry|PlayBook/;
...
google.bookmarkbubble.Bubble.prototype.IOS_VERSION_USERAGENT_REGEX_ = /OS (\d)_(\d)(?:_(\d))?/;
google.bookmarkbubble.Bubble.prototype.ANDROID_VERSION_USERAGENT_REGEX_ = /Android (\d)\.(\d)(?:\.(\d))?/;
...
google.bookmarkbubble.Bubble.prototype.rememberDismissal_ = function() {
  if (window.localStorage) {
    try {
      var key = this.LOCAL_STORAGE_PREFIX + this.DISMISSED_ + identifier;
